...
- **双列表管理**：将项目分为编译列表和未编译列表
- **复选框选择**：点击项目前的复选框快速切换项目的编译状态
- **拖放操作**：在同一列表内拖动来控制构建顺序
...
点击 **构建** 按钮（▶️）开始按指定顺序构建**编译列表**中的所有项目。
...
2. **生成命令**：为**编译列表**中的每个项目创建转换命令
3. **运行转换**：执行 cbp2clang 生成 compile_commands.json，用于 clangd 插件索引项目
