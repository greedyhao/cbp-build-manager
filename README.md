...
- **CBP 文件树与搜索**：工程文件按公共目录树展示；搜索支持路径/文件名部分匹配，按 Enter 定位到第一个匹配文件，方便确认或移出工程
...
- **文件列表**：Unit 按工程文件的公共目录树展示；不会把 CBP 路径中的 `..` 直接显示在界面上
- **搜索工程文件**：输入关键字会实时过滤路径和文件名；允许部分匹配，按 **Enter** 后自动展开父目录、滚动并高亮第一个匹配文件，但不会打开外部源文件。没有结果时会显示提示
- **搜索清除**：按 **Escape** 清空搜索并恢复完整文件树
- **文件操作**：添加文件使用顶部工具栏，新增 `<Unit>` 与官方 Code::Blocks 一致写入相对 `.cbp` 所在目录的相对路径（如 `../../platform/...`），并沿用已有 Unit 的 `<Option compilerVar="CC" />` 风格；对文件右键选择“移出工程”，确认后只删除 CBP 中的 `<Unit>` 引用
