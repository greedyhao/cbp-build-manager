...
} from "../../services/CompileCommandsMerger";
...
suite("CompileCommandsMerger Test Suite", () => {
  ...
  test("should skip merge when less than 2 valid CBP files exist", async () => {
    ...
    assert.strictEqual(result, true);
    assert.ok(
      logMessages.some((msg) => msg.includes("有效的 CBP 项目数量少于2个")),
    );
  });
  ...
  test("should attempt merge when multiple CBP files exist", async () => {
    ...
    assert.ok(
      logMessages.some((msg) => msg.includes("合并 compile_commands.json")),
    );
  ...
  test("should correctly identify target as last project", async () => {
    ...
    assert.ok(
      logMessages.some(
        (msg) => msg.includes("project3") && msg.includes("目标项目"),
      ),
    );
  });
});
