...
export class CbpDataManager {
  ...
  private loadState() {
    ...
    try {
      ...
      // 重建对象
      this.buildQueue = savedQueuePaths
        .map((fsPath) => {
          // 检查文件是否仍存在
  ...
  protected saveState() {
    ...
    try {
      ...
      this.buildQueue.forEach(
        (p) =>
          (checkState[p.fsPath] =
            p.checkboxState === vscode.TreeItemCheckboxState.Checked),
      );
  ...
  async scanCompileCommands() {
    ...
    // 清理已经不存在的路径
    this.compileCommandsOrder = this.compileCommandsOrder.filter((p) =>
      currentPaths.has(p),
    );
  ...
  updateCompileCommandsCheckState(
    item: CompileCommandsItem,
  ...
  moveCompileCommandsItem(
    sourceItems: CompileCommandsItem[],
    target?: CompileCommandsItem,
  ) {
    const pathsToMove = sourceItems.map((s) => s.fsPath);
    // 从顺序列表中移除
    const newOrder = this.compileCommandsOrder.filter(
      (p) => !pathsToMove.includes(p),
    );

    if (target) {
      const insertIndex = newOrder.indexOf(target.fsPath);
      newOrder.splice(
        insertIndex === -1 ? newOrder.length : insertIndex,
        0,
        ...pathsToMove,
      );
    } else {
  ...
  moveQueueItem(sourceItems: CbpProjectItem[], target?: CbpProjectItem) {
    // 提取需要移动的项目
    const itemsToMove = sourceItems.filter((s) =>
      this.buildQueue.some((inQ) => inQ.fsPath === s.fsPath),
    );

    // 从原数组中移除
    const newQueue = this.buildQueue.filter(
      (p) => !itemsToMove.some((m) => m.fsPath === p.fsPath),
    );
