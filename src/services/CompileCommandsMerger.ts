 ...
 */
export async function checkMergeCommandSupport(
...
export async function mergeCompileCommands(
  ...
  const existingCbpPaths = cbpPaths.filter((p) => {
    try {
      ...
      debugLog(
        `\x1b[36m[调试] 检查 CBP 文件 ${path.basename(p)}: ${exists ? "存在" : "不存在"}\x1b[0m\n`,
      );
      return exists;
    } catch {
      return false;
  ...
  if (existingCbpPaths.length <= 1) {
    debugLog(
      `\x1b[36m[调试] 找到 ${existingCbpPaths.length} 个有效的 CBP 文件\x1b[0m\n`,
    );
  ...
  try {
    ...
    debugLog(
      `\x1b[36m[调试] 完整命令: ${cbp2clangPath} ${mergeArgs.join(" ")}\x1b[0m\n`,
    );
    ...
    if (result.success) {
      ...
      log(
        `\x1b[32m合并完成: 已将 ${sourceCbpPaths.length} 个项目的 compile_commands.json 合并到 "${path.basename(targetCbpPath)}"\x1b[0m\n`,
      );
      return true;
    } else {
      log(
        `\x1b[33m警告: 合并 compile_commands.json 失败: ${result.error}\x1b[0m\n`,
      );
      if (result.stdout) {
        log(`\x1b[90m${result.stdout}\x1b[0m\n`);
      }
    ...
    log(
      `\x1b[33m警告: 合并 compile_commands.json 时发生异常: ${(error as Error).message}\x1b[0m\n`,
    );
    return false;
  }
}
...
export async function mergeCompileCommandsFiles(
  ...
  if (compileCommandsPaths.length < 2) {
    log(
      "\x1b[33m至少需要 2 个 compile_commands.json 文件（1 个目标 + 至少 1 个源）\x1b[0m\n",
    );
    return false;
  }
  ...
  const relPath = (p: string) => {
    const dir = displayRoot
      ? path.relative(displayRoot, path.dirname(p))
      : path.dirname(p);
