...
export {
  ...
  checkMergeCommandSupport,
  mergeCompileCommandsFiles,
} from "./services/index";
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code: number) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `Failed to check cbp2clangd version: ${error || `Exit code ${code}`}`,
          ),
        );
      }
    });
...
async function checkAndPromptSave(): Promise<boolean> {
  // 获取所有未保存的文档
  const unsavedDocs = vscode.workspace.textDocuments.filter(
    (doc) => doc.isDirty,
  );
...
function checkAndUpdateNinjaPath(config: vscode.WorkspaceConfiguration) {
  ...
  try {
    ...
    terminal.write(
      `路径存在，类型: ${stats.isDirectory() ? "文件夹" : "文件"}\n`,
    );
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.onDidCloseTerminal((terminal) => {
      ...
      if (
        terminal.name === "CBP Build Manager" &&
        g_terminal &&
        terminal === g_terminal
      ) {
  ...
  const compileCommandsTreeView = vscode.window.createTreeView(
    "cbpCompileCommands",
    {
  ...
  // 1. 刷新
  context.subscriptions.push(
    vscode.commands.registerCommand("cbp-build-manager.refreshProjects", () => {
      manager.scanWorkspace();
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.checkCbp2clangVersion",
      async () => {
        const config = vscode.workspace.getConfiguration("cbpBuildManager");
        ...
        try {
          ...
          const isCompatible = compareVersions(
            version,
            MIN_REQUIRED_CBP2CLANG_VERSION,
          );

          if (isCompatible) {
            vscode.window.showInformationMessage(
              `cbp2clangd 版本: ${version} (满足要求)`,
            );
            terminal.write(
              `cbp2clangd 版本: ${version} (满足要求，最小要求版本: ${MIN_REQUIRED_CBP2CLANG_VERSION})\n`,
            );
          } else {
            vscode.window.showWarningMessage(
              `cbp2clangd 版本: ${version} (低于最小要求版本 ${MIN_REQUIRED_CBP2CLANG_VERSION})`,
            );
            terminal.write(
              `cbp2clangd 版本: ${version} (警告: 低于最小要求版本 ${MIN_REQUIRED_CBP2CLANG_VERSION})\n`,
            );
          }
        } catch (error) {
          vscode.window.showErrorMessage(
            `检查 cbp2clangd 版本失败: ${(error as Error).message}`,
          );
          terminal.write(
            `检查 cbp2clangd 版本失败: ${(error as Error).message}\n`,
          );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.filterByChip",
      async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.refreshCompileCommands",
      () => {
        manager.scanCompileCommands();
      },
    ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.mergeCompileCommands",
      async () => {
        const terminal = createOrShowTerminal();
        const checkedItems = manager
        ...
        if (checkedItems.length < 2) {
          terminal.write(
            "至少需要勾选 2 个 compile_commands.json 才能合并\r\n",
          );
          vscode.window.showInformationMessage(
            "至少需要勾选 2 个 compile_commands.json 才能合并",
          );
        ...
        terminal.write(
          `\x1b[36m=== 合并 ${files.length} 个 compile_commands.json ===\x1b[0m\r\n`,
        );
        const success = await mergeCompileCommandsFiles(
          files,
          cbp2clangPath,
          debugMode,
        ...
        if (success) {
          vscode.window.showInformationMessage(
            `成功合并 ${files.length} 个 compile_commands.json`,
          );
        } else {
          terminal.write(`\x1b[31m合并失败\x1b[0m\r\n`);
          vscode.window.showErrorMessage(
            "合并 compile_commands.json 失败，请检查终端输出",
          );
        }
      },
    ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.removeFromBuild",
      (item?: any) => {
        if (item) {
          // 如果右键点击单个项目，删除该项目
  ...
  // 4.5 停止构建
  context.subscriptions.push(
    vscode.commands.registerCommand("cbp-build-manager.stopBuild", () => {
      const terminal = createOrShowTerminal();
      if (stopCurrentBuild()) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.buildSelected",
      async () => {
        if (isBuilding) {
        ...
        try {
          ...
          const selectedProjects = queue.filter(
            (p) => p.checkboxState === vscode.TreeItemCheckboxState.Checked,
          );
          ...
          const config = vscode.workspace.getConfiguration("cbpBuildManager");
          const cbp2clangPath = config.get<string>(
            "cbp2clangPath",
            "cbp2clang",
          );
          const convertCommandTemplate = config.get<string>(
            "convertCommand",
            "{cbp2clang} {cbpFile} {compileCommands} -l ld",
          );
          const buildScript = config.get<string>("buildCommand", "./build.bat");
          const ninjaPath = config.get<string>("ninjaPath", "");
          const noHeaderInsertion = config.get<boolean>(
            "noHeaderInsertion",
            false,
          );
          const debugMode = config.get<boolean>("debug", false);
          ...
          for (const project of selectedProjects) {
            ...
            try {
              ...
              if ((error as Error).message === "STOPPED") {
                terminal.write(
                  `\x1b[33m>>> 项目 ${project.label} 已停止\x1b[0m\n`,
                );
                stopped = true;
                break;
              }
              terminal.write(
                `\x1b[31m!!! 项目 ${project.label} 失败: ${error}\x1b[0m\n`,
              );
              if (stopOnFailure) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.rebuildSelected",
      async () => {
        if (isBuilding) {
          vscode.window.showInformationMessage("已有构建任务正在运行。");
          return;
        ...
        try {
          ...
          for (const project of selectedProjects) {
            ...
            try {
              ...
              terminal.write(
                `\x1b[32m>>> 项目 ${project.label} 重新编译完成.\x1b[0m\n`,
              );
            } catch (error) {
              if ((error as Error).message === "STOPPED") {
                terminal.write(
              ...
              terminal.write(
                `\x1b[31m!!! 项目 ${project.label} 重新编译失败: ${error}\x1b[0m\n`,
              );
              if (stopOnFailure) {
                terminal.write(`\x1b[31m>>> 编译失败，停止后续项目\x1b[0m\n`);
                break;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "cbp-build-manager.cleanSelected",
      async () => {
        if (isBuilding) {
          vscode.window.showInformationMessage("已有构建任务正在运行。");
          return;
        ...
        try {
          ...
          for (const project of selectedProjects) {
            ...
            try {
              ...
              terminal.write(
                `\x1b[32m>>> 项目 ${project.label} 清理完成.\x1b[0m\n`,
              );
            } catch (error) {
              if ((error as Error).message === "STOPPED") {
                terminal.write(
              ...
              terminal.write(
                `\x1b[31m!!! 项目 ${project.label} 清理失败: ${error}\x1b[0m\n`,
              );
