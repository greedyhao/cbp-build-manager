...
export class ProjectLibraryProvider implements vscode.TreeDataProvider<
  CbpProjectItem | DirectoryItem
> {
  ...
  private _onDidChangeTreeData = new vscode.EventEmitter<
    CbpProjectItem | DirectoryItem | undefined | null | void
  >();
  ...
  ): vscode.ProviderResult<(CbpProjectItem | DirectoryItem)[]> {
    const availablePaths = this.manager.getAvailableItems(); // 仅获取未添加的项目

    if (!element) {
      // 根目录：计算所有可用项目的顶层结构
      return this.buildTreeLevel(
        availablePaths,
