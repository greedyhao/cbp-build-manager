...
export class CompileCommandsProvider
  implements
    vscode.TreeDataProvider<CompileCommandsItem>,
    vscode.TreeDragAndDropController<CompileCommandsItem>
{
  ...
  private _onDidChangeTreeData = new vscode.EventEmitter<
    CompileCommandsItem | undefined | null | void
  >();
  ...
  getChildren(
    element?: CompileCommandsItem,
  ): vscode.ProviderResult<CompileCommandsItem[]> {
    if (element) {
  ...
  handleDrag(
    source: readonly CompileCommandsItem[],
    dataTransfer: vscode.DataTransfer,
  ): void {
    const itemPaths = source.map((item) => item.fsPath);
    dataTransfer.set(
      "application/vnd.code.tree.cbpCompileCommands",
      new vscode.DataTransferItem(itemPaths),
    );
  }

  handleDrop(
    target: CompileCommandsItem | undefined,
    dataTransfer: vscode.DataTransfer,
  ): void {
